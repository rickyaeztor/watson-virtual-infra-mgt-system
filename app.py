...
@app.route('/api/test_watson', methods=['GET'])
def test_watson():
    td = TradeoffAnalyticsV1(username = '2f524550-89d1-4dd7-9394-e395c2d4a75a', password = 'VTfGUyMJGES0')

    with open(os.path.join(os.path.dirname(__file__),'resources/esxhosts.json')) as problem_json:
          dilemma = td.dilemmas(json.load(problem_json), generate_visualization=True, find_preferable_options=True)
    print(json.dumps(dilemma, indent=2))
    return json.dumps(dilemma, indent=2)
